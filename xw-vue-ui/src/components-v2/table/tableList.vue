    ...
    <div>
        ...
        <div class="tableContainer">
            <div class="tableBox" @scroll="scrollHandler">
                <table class="table">
                    <header-section
                        ...
                        :show-ck="showCk"
                        :origin-cols="originCols"
                        :accpet-h-b-notice="accpetHBNotice"
                        :notice-change-cols="noticeChangeCols"
                        :ck="state.ck"
                        :actions="actions"
                        :cols="state.cols"
            ...
            <div v-show="isLoading" class="tableMask">
                <img
                    class="tableLoadingImg"
                    src="//p2-nec.static.pub/fes/cms/2020/04/19/t960paupk2x9wzsw1y79dcmpeho847108450.gif"
                />
            </div>
        </div>
    </div>
    ...
    data() {
        return {
            ...
            state: {
                ...
                ck: false,
                pageOption: {
                    index: this.options.pageOption.index
                        ? this.options.pageOption.index
                        : 1,
                    size: this.options.pageOption.size
                        ? this.options.pageOption.size
                        : 10,
                    count: 0,
                    total: 0,
                },
    ...
    methods: {
        setData(res) {
            if (res.data && res.data instanceof Array && res.data.length != 0) {
                ...
                this.noResultCb();
                console.log(
                    "The data source is empty, please check  analysis , getUrl and pageOption"
            );
            }
        },
        /**
        ...
        getData: function (index) {
            ...
            if (this.ajaxType == "get") {
                ...
                url +=
                    suffix +
                    this.options.pageOption.indexKey +
                    "=" +
                    index +
                    "&" +
                    this.options.pageOption.sizeKey +
                    "=" +
                    size;
                tmpPromise = this.ajax.get(url);
            } else {
                let tmpData = this.options.getParams
        ...
        accpetHBNotice: function (hData, bData) {
            ...
            if (bData) {
                ...
                let flag =
                    items.items.length == this.state.data.length ? true : false;
                this.state = {
                    data: bData.data,
                    ck: flag,
                    cols: this.state.cols,
...
.tableContainer {
    ...
    box-shadow: 3px 0px 12px 0px rgba(112, 112, 112, 0.2);
